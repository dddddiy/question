...
def getQuestionAnalysis(questionId):
    ...
    cursor.execute(
        'select A.id,count(B.submitId),A.title from (select * from myAdmin_options where questionId=%s) A left join myAdmin_answer B on A.id=B.answer group by A.id' % questionId)
